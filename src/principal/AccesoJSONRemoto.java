...
public class AccesoJSONRemoto {
	...
	private String SERVER_PATH, GET_VUELO, SET_VUELO, compra; // Datos de la conexion
	...
	public HashMap<Integer, Vuelo> leerVuelos() {
		hmLeer = new HashMap<Integer, Vuelo>();
		// Cambiar la posicion del HM por el id pero eso hace que solo se descargue el
		// del ultimo servicio que llamas
	...
	public HashMap<Integer, Vuelo> buscarVuelo(Vuelo vuelo) {
		hmBuscar = new HashMap<Integer, Vuelo>();
		numeroVuelosBuscar = 0;
		buscarVuelosRyanair1(vuelo);
		buscarVuelosIberia(vuelo);
		return hmBuscar;
	}

	public void leerVuelosIberia1() {// Servdor PHP
				...
				if (estado.equals("ok")) {
					...
					if (array.size() > 0) {
						...
						for (int i = 0; i < array.size(); i++) {
							...
							nuevoVuelo = new Vuelo(id, origen, destino, fecha, precio, plazasTotales, plazasLibres,
									"ry", numeroVuelosLeer);
	...
	public void leerVuelosRyanair() {// Aplicacion Web
		...
		try {
			...
			for (int i = 0; i < respuesta.size(); i++) {
				...
				Vuelo vueloAux = new Vuelo(Integer.parseInt(vueloJson.get("idVuelo").toString()),
						vueloJson.get("origen").toString(), vueloJson.get("destino").toString(),
						vueloJson.get("diayhora").toString(), Double.parseDouble(vueloJson.get("precio").toString()),
						Integer.parseInt(vueloJson.get("plazas_totales").toString()),
						Integer.parseInt(vueloJson.get("plazas_libres").toString()), "ib", numeroVuelosLeer);
	...
	public void buscarVuelosRyanair1(Vuelo auxVuelo) {// Aplicacion Web
		...
		try {
			...
			for (int i = 0; i < respuesta.size(); i++) {
				...
				Vuelo vueloAux = new Vuelo(Integer.parseInt(vueloJson.get("idVuelo").toString()),
						...
						vueloJson.get("diayhora").toString(), Double.parseDouble(vueloJson.get("precio").toString()),
						Integer.parseInt(vueloJson.get("plazas_totales").toString()),
						Integer.parseInt(vueloJson.get("plazas_libres").toString()), "ib", numeroVuelosBuscar);
			...
			/*
			 * if (respuesta == null) { // Si hay algún error de parseo (json // incorrecto
			 * porque hay algún caracter // raro, etc.) la respuesta será null
			 * System.out.println("El json recibido no es correcto. Finaliza la ejecución");
			 * System.exit(-1); } else { // El JSON recibido es correcto
			 * 
			 * // Sera "ok" si todo ha ido bien o "error" si hay algún problema String
			 * estado = (String) respuesta.get("estado"); if (estado.equals("ok")) {
			 * 
			 * System.out.println("Buscado vuelo enviado por JSON Remoto");
			 * 
			 * } else { // Hemos recibido el json pero en el estado se nos // indica que ha
			 * habido algún error
			 * 
	...
	public void buscarVuelosIberia(Vuelo auxVuelo) {// Servdor PHP
		String url = SERVER_PATH + SET_VUELO;
		...
		try {
			...
			for (int i = 0; i < vuelos.size(); i++) {
				...
				Vuelo vueloAux = new Vuelo(Integer.parseInt(vueloJson.get("idVuelo").toString()),
						...
						vueloJson.get("diayhora").toString(), Double.parseDouble(vueloJson.get("precio").toString()),
						Integer.parseInt(vueloJson.get("plazas_totales").toString()),
						Integer.parseInt(vueloJson.get("plazas_libres").toString()), "ry", numeroVuelosBuscar);
				hmBuscar.put(numeroVuelosBuscar, vueloAux);
				numeroVuelosBuscar++;
			}
	...
	public void comprarRY(Vuelo vuelo) {
		// PHP
		JSONObject objVuelo = new JSONObject();
		JSONObject objPeticion = new JSONObject();

		objVuelo.put("idvuelo", vuelo.getId());


		objPeticion.put("peticion", "comprar");
		objPeticion.put("vueloUpdate", objVuelo);
		
		String json = objPeticion.toJSONString();

		String url = SERVER_PATH + compra;
	...
	public void comprarIB(Vuelo vuelo) {
		// WS
	}

	public void comprarVuelo(int numVuelo) {

		Vuelo auxVuelo = hmBuscar.get(numVuelo);

		if (auxVuelo.getCompañia().equals("ry")) {
			comprarRY(auxVuelo);
		} else if (auxVuelo.getCompañia().equals("ib")) {
			comprarIB(auxVuelo);
		}
